...
class EdgeSuite extends org.scalatest.FunSuite {
  ...
  test("retrieve edge's sourde-edge-target") {
    ...
    /* Retrieving edge */
    implicit def retrieveMemberOf(e: memberOf.type) = new user.RetrieveManyOutEdge(memberOf) {
    ...
    implicit def retrieveOwns(e: owns.type) = new user.RetrieveOneOutEdge(owns) {
