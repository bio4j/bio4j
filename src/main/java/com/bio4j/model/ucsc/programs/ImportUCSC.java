...
public abstract class ImportUCSC<I extends UntypedGraph<RV, RVT, RE, RET>, RV, RVT, RE, RET> {

    private static final Logger logger = Logger.getLogger("ImportUCSC");
    private static FileHandler fh;

    protected abstract UCSCGraph<I, RV, RVT, RE, RET> config(String dbFolder, String propertiesFile);
    ...
    public void importUCSC(String[] args) {
        ...
        if (args.length != 3) {
            ...
            UCSCGraph<I, RV, RVT, RE, RET> UCSCGraph = config(dbFolder, propertiesFile);
            ...
            try {
                ...
                for (int i = 1; i <= 22; i++) {
                    ...
                    UCSCChromosome<I, RV, RVT, RE, RET> chromosome = UCSCGraph.addVertex(UCSCGraph.UCSCChromosome());
                    chromosome.set(UCSCGraph.UCSCChromosome().id, "chr" + i);

                    chromosomeCounter++;

                    if ((chromosomeCounter % limitForTransaction) == 0) {
                        UCSCGraph.raw().commit();
                    }
                }
                ...
                Logger.getLogger(ImportUCSC.class.getName()).log(Level.SEVERE, null, ex);
